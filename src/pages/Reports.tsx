  ...
  const renderGstr3bReport = () => {
    ...
    return (
      <div className="gstr3b-working-report-wrapper" style={{ padding: 0, border: 'none', boxShadow: 'none' }}>
        {/* PAGE HEADER */}
        <div className="gstr3b-header-container" style={{ display: 'flex', flexDirection: 'column', gap: '16px', borderBottom: '2px solid #2f3e33', paddingBottom: '16px', marginBottom: '24px' }}>
          {/* Top Row: Logo & Company details */}
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
            {settings.showLogo && settings.logo && (
              <div style={{ flexShrink: 0 }}>
                <img src={settings.logo} alt="Company Logo" style={{ maxHeight: '90px', maxWidth: '140px', objectFit: 'contain', borderRadius: '8px' }} />
              </div>
            )}
            <div>
              <h1 style={{ margin: 0, fontSize: '20px', fontWeight: 800, color: '#2f3e33' }}>
                {settings.businessName || 'Daivyog Engineering Work'}
              </h1>
              {settings.showAddress && (
                <div style={{ fontSize: '11px', color: '#5d6b5e', marginTop: '4px', lineHeight: '1.4' }}>
                  {getFullAddress(settings) || 'Shop No. 12-14, Krishi Mandi Complex, Mandi Area, Pipariya, Taluka: Pipariya, Hoshangabad, Madhya Pradesh - 461775'}
                </div>
              )}
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 16px', marginTop: '4px', fontSize: '11px', color: '#5d6b5e' }}>
                {settings.showGstin && settings.gstin && (
                  <span style={{ fontWeight: 700, color: '#2f3e33' }}>GSTIN: {settings.gstin}</span>
                )}
                {settings.phone && (
          ...
          {/* Bottom Row: Report Meta in Rows */}
          <div style={{ borderTop: '1px dashed #e2e9e0', paddingTop: '12px' }}>
            <div style={{ fontSize: '10px', fontWeight: 700, color: '#10b981', letterSpacing: '1px', textTransform: 'uppercase' }}>
              FINANCIAL GST REPORT
            </div>
            <h2 style={{ margin: '4px 0 8px 0', fontSize: '18px', fontWeight: 900, color: '#2f3e33', textTransform: 'uppercase' }}>
              GSTR-3B WORKING REPORT
            </h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px', color: '#5d6b5e' }}>
              <div><strong>Return Period:</strong> {returnPeriod()}</div>
              <div><strong>Generated On:</strong> {generatedOn}</div>
              <div><strong>Generated By:</strong> {settings.ownerName || 'Kunal Chaudhari'}</div>
            </div>
          </div>
        </div>
    ...
    <div style={{ animation: 'fadeIn 0.2s ease-out' }}>
      ...
      <div className={`pdf-print-wrapper ${activeReport.startsWith('gstr') && activeReport !== 'gstr3b' ? 'landscape-report' : 'portrait-report'}`} style={{ position: 'fixed', left: '-9999px', top: 0, width: activeReport.startsWith('gstr') && activeReport !== 'gstr3b' ? '1122px' : '794px', height: 'auto', overflow: 'visible', zIndex: -1, pointerEvents: 'none', visibility: 'hidden' }}>
        <div id="pdf-report-printout" style={{
          ...
          {/* Header Block (Unified Invoice PDF Header style, hidden for gstr3b since gstr3b renders its own previous custom header) */}
          {activeReport !== 'gstr3b' && (
