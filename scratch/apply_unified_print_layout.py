...
# 3. New Invoice bottom block: from BANK DETAILS header to footer message end
new_bottom_inv = """                    <div style={{ marginBottom: "8px" }}>
...
# Replace the Invoice bank details block
start_bank_inv = content.find('                    <div style={{ display: "flex", flexDirection: "column", gap: "3px", marginTop: "4px" }}>\n                      <div style={{ fontSize: "9.5px"')
end_bank_inv = content.find('                    </div>', start_bank_inv) + len('                    </div>')

if start_bank_inv != -1 and end_bank_inv != -1:
    content = content[:start_bank_inv] + new_bottom_inv + content[end_bank_inv:]
    print("SUCCESS: Updated Invoice bank details layout to a clean aligned table.")
else:
    print("WARNING: Invoice bank details container not found by exact string!")


# 4. Replace the Quotation bank details block
start_bank_quot = content.find('                      <div style={{ display: "flex", flexDirection: "column", gap: "3px", marginTop: "4px" }}>\n                        <div style={{ fontSize: "9.5px"')
end_bank_quot = content.find('                      </div>', start_bank_quot) + len('                      </div>')

new_bottom_quot = """                      <div style={{ marginBottom: "8px" }}>
                        <h5 className="invoice-terms-title" style={{ marginBottom: "4px" }}>BANK DETAILS:</h5>
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "9.5px", color: "var(--text-muted)", marginTop: "4px" }}>
                          <tbody>
                            <tr>
...
if start_bank_quot != -1 and end_bank_quot != -1:
    content = content[:start_bank_quot] + new_bottom_quot + content[end_bank_quot:]
    print("SUCCESS: Updated Quotation bank details layout to a clean aligned table.")
else:
    print("WARNING: Quotation bank details container not found by exact string!")


# 5. Let's find the SVG & Signatures block for Invoice and replace with a unified/fixed layout with a Flex Spacer!
...
if start_svg_inv != -1 and end_sig_inv != -1:
    ...
    print("SUCCESS: Updated Invoice SVG and signatures layout to a clean unified block with spacer.")
else:
    print("ERROR: Invoice SVG / Signatures block not found by index!", start_svg_inv, end_sig_inv)


# 6. Let's find the SVG & Signatures block for Quotation and replace with a unified/fixed layout with a Flex Spacer!
...
# Find the start/end in content for Quotation
start_svg_quot = content.find('{/* SVG Illustration centered */}', start_bank_quot)
...
if start_svg_quot != -1 and end_sig_quot != -1:
    ...
    print("SUCCESS: Updated Quotation SVG and signatures layout to a clean unified block with spacer.")
else:
    print("ERROR: Quotation SVG / Signatures block not found by index!", start_svg_quot, end_sig_quot)
