...
def clean_escapes(s):
    if not s:
        return ""
    # Strip leading and trailing double quotes independently
    if s.startswith('"'):
        s = s[1:]
    if s.endswith('"'):
        s = s[:-1]
    # Unescape JSON escape characters
    s = s.replace("\\\\", "\\")
    s = s.replace("\\n", "\n")
    s = s.replace("\\r", "\r")
    s = s.replace("\\t", "\t")
...
# Reset Sales.tsx to HEAD first
os.system("git checkout HEAD -- c:\\Users\\Admin\\Desktop\\AgriBiz\\src\\pages\\Sales.tsx")

# Load clean base file content
...
for step in steps_to_apply:
    ...
    # Handle multi replacement chunks
    elif "ReplacementChunks" in args:
        print(f"Applying Multi-Chunk Step {step}...")
        for chunk in args["ReplacementChunks"]:
            target = clean_escapes(chunk["TargetContent"])
            replacement = clean_escapes(chunk["ReplacementContent"])
